...
import { LockClosedIcon, LockOpenIcon, CogIcon, GiftIcon, CloudIcon } from './icons/Icons';
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center justify-between w-full md:w-auto">
                ...
                <div className="flex items-center gap-2 md:hidden">
                    <button
                        onClick={onSyncClick}
                        className={`p-2 rounded-lg transition-colors ${isOnline ? 'text-sky-500 hover:bg-sky-50 dark:hover:bg-sky-900/20' : 'text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'}`}
                    >
                        <CloudIcon />
                    </button>
                   <button
                     onClick={onRewardsClick}
                     className="p-2 rounded-lg text-amber-500 hover:bg-amber-50 dark:text-amber-400 dark:hover:bg-amber-900/20"
                   >
                     <GiftIcon />
                   </button>
                   <ThemeToggle />
            ...
            <div className="flex items-center justify-between md:justify-end gap-3 md:gap-4 w-full md:w-auto">
                <div className="hidden md:flex items-center gap-2">
                    <button 
                        onClick={onSyncClick}
                        className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-all ${isOnline ? 'text-sky-600 hover:bg-sky-50 dark:text-sky-400 dark:hover:bg-sky-900/20' : 'text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800'}`}
                        title="Sincronização"
                    >
                        <CloudIcon />
                    ...
                    <button 
                        onClick={onRewardsClick}
                        className="flex items-center gap-2 px-3 py-2 rounded-lg text-amber-600 hover:bg-amber-50 dark:text-amber-400 dark:hover:bg-amber-900/20 transition-all"
                        title="Ver Benefícios"
                    >
                        <GiftIcon />
                        <span className="text-xs font-bold uppercase">Benefícios</span>
                    </button>
