...
export default function App() {
  ...
  const { rewards, updateRewardDescription, updateRewardThreshold } = useRewards();
  ...
  const weeklyStats = useMemo(() => {
    ...
    const score = totalTaskPoints - totalFaults;

    // Calculate Max Possible Points for the week (Sum of all tasks * 7 days)
    const maxPotentialPoints = tasks.reduce((sum, task) => sum + task.points, 0) * 7;

    return { score, maxPotentialPoints };
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans">
      ...
      <RewardsModal 
        ...
        rewards={rewards}
        currentScore={weeklyStats.score}
        maxScore={weeklyStats.maxPotentialPoints}
        isParentMode={isParentMode}
        onUpdateReward={updateRewardDescription}
        onUpdateThreshold={updateRewardThreshold}
      />
